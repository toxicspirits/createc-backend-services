...
def insert_prepared_requests(lambda_events):
    ...
    query = """
    INSERT INTO logging_scraping_pipeline.request_details(company_id, operation_type, request_json) 
    VALUES(%s,%s,%s)
...
def insert_prepared_amz_requests(lambda_events):
    ...
    cur = rds_conn.cursor(row_factory=dict_row)
    query = """
    INSERT INTO logging_scraping_pipeline.request_details(company_id, amazon_profile_id, operation_type, request_json) 
    VALUES(%s,%s,%s,%s)
    RETURNING request_id;
    """
    for event in lambda_events:
