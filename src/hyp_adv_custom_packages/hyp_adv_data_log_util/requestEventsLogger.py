...
def insert_prepared_events(lambda_events):
    ...
    query = """
    INSERT INTO logging_data_pipeline.request_events_details(request_id, event) 
    VALUES(%s, %s)
    RETURNING event_id;
    """
...
def update_fetch_status(request_id, event_id, status, **kwargs):
    ...
    query = f"""
            UPDATE logging_data_pipeline.request_events_details SET fetch_status=(%s), fetch_log=(%s)
...
def update_process_status(request_id, event_id, status, **kwargs):
    ...
    query = f"""
            UPDATE logging_data_pipeline.request_events_details SET process_status=(%s), process_log=(%s)
            WHERE request_id = {request_id} AND event_id = {event_id};
            """
